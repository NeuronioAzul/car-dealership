...
function run_migration($service, $migration_file, $db_name) {
    ...
    $cmd = "docker-compose exec -T mysql mysql -u $db_user -p$db_password $db_name < \"$file_path\" 2>&1";
...
function create_database($db_name) {
    ...
    $cmd = "docker-compose exec -T mysql mysql -u $db_user -p$db_password -e \"SHOW DATABASES LIKE '$db_name';\" 2>/dev/null";
    exec($cmd, $output, $status);
    if ($status === 0 && count($output) > 1 && trim($output[1]) === $db_name) {
        log_success("Banco já existe: $db_name");
        return true;
    }
    // Se não existir, cria o banco
    log_info("Banco não encontrado, criando: $db_name");
    // Cria o banco de dados
    // get user and password from .env file
    $db_user = getenv('DB_USERNAME');
    $db_password = getenv('DB_PASSWORD');
    // Use -T para evitar problemas com tty em ambientes não interativos
    // Use -e para executar o comando diretamente
    $cmd = "docker-compose exec -T mysql mysql -u $db_user -p$db_password -e \"CREATE DATABASE IF NOT EXISTS $db_name;\" 2>/dev/null";
    exec($cmd, $output, $status);
    if ($status === 0) {
...
// pergunta se tem certeza
    ...
    foreach ($databases as $db) {
        log_info("Excluindo banco de dados: $db");
        // get user and password from .env file
        $db_user = getenv('DB_USERNAME');
        $db_password = getenv('DB_PASSWORD');
        // Verifica se o banco existe antes de tentar excluir
        $cmd = "docker-compose exec -T mysql mysql -u $db_user -p$db_password -e \"SHOW DATABASES LIKE '$db';\" 2>/dev/null";
        exec($cmd, $output, $status);
        if ($status !== 0 || count($output) <= 1 || trim($output[1]) !== $db) {
            log_warning("Banco não encontrado, pulando exclusão: $db");
            continue;
        }
        // Exclui o banco de dados
        log_info("Banco encontrado, excluindo: $db");
        $db_user = getenv('DB_USERNAME');
        $db_password = getenv('DB_PASSWORD');
        $cmd = "docker-compose exec -T mysql mysql -u $db_user -p$db_password -e \"DROP DATABASE IF EXISTS $db;\" 2>/dev/null";
        exec($cmd, $output, $status);
        if ($status === 0) {
...
foreach ($databases as $db) {
    ...
    $db_user = getenv('DB_USERNAME');
    $db_password = getenv('DB_PASSWORD');
    $cmd = "docker-compose exec -T mysql mysql -u $db_user -p$db_password -e \"SELECT COUNT(*) as count FROM information_schema.tables WHERE table_schema = '$db';\" 2>/dev/null";
    exec($cmd, $output, $status);
